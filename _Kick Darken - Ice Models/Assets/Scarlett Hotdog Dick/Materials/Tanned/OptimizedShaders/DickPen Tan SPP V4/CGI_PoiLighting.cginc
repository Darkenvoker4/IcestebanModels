...
void calculateBasePassLightMaps()
{
        ...
        shadowStrength = 1;
        #ifndef OUTLINE
            shadowStrength = POI2D_SAMPLER_PAN(_LightingShadowMask, _MainTex, poiMesh.uv[(0.0 /*_LightingShadowMaskUV*/)], float4(0,0,0,0)) * (0.2 /*_ShadowStrength*/);
        #else
        ...
        switch((0.0 /*_LightingRampType*/))
        {
            ...
            {
                poiLight.rampedLightMap = saturate(1 - smoothstep((0.0 /*_LightingGradientStart*/) - .000001, (1.0 /*_LightingGradientEnd*/), 1 - poiLight.lightMap));
                ...
                if (_LightingIgnoreAmbientColor)
                {
                    poiLight.finalLighting = lerp((directLighting * shadowColor * poiLight.occlusion), (directLighting), saturate(poiLight.rampedLightMap + 1 - (0.2 /*_ShadowStrength*/)));
                }
                else
                {
                    poiLight.finalLighting = lerp((indirectLighting * shadowColor * poiLight.occlusion), (directLighting), saturate(poiLight.rampedLightMap + 1 - (0.2 /*_ShadowStrength*/)));
                }
            }
            break;
            case 2:
        ...
        if ((3.0 /*_LightingMode*/) == 2) // Wrapped
        {
            ...
            {
                poiLight.rampedLightMap = lerp(float4(1,1,1,1) * lerp(poiLight.indirectLighting, 1, _LightingIgnoreAmbientColor), float3(1, 1, 1), saturate(1 - smoothstep((0.0 /*_LightingGradientStart*/) - .000001, (1.0 /*_LightingGradientEnd*/), 1 - poiLight.lightMap)));
                poiLight.rampedLightMap = lerp(float3(1, 1, 1), poiLight.rampedLightMap, shadowStrength.r);
            }
            poiLight.finalLighting = (poiLight.indirectLighting + poiLight.directLighting) * saturate(poiLight.rampedLightMap + 1 - (0.2 /*_ShadowStrength*/));
        }
        if (!(0.0 /*_LightingUncapped*/))
        {
...
float3 calculateNonImportantLighting(float attenuation, float attenuationDotNL, float3 albedo, float3 lightColor, half dotNL, half correctedDotNL)
{
    ...
    else //if((1.0 /*_LightingAdditiveType*/) == 2) // Wrapped
    {
        ...
        poiLight.rampedLightMap = lerp(float4(1,1,1,1), float3(1, 1, 1), saturate(1 - smoothstep((0.0 /*_LightingGradientStart*/) - .000001, (1.0 /*_LightingGradientEnd*/), 1 - uv)));
    ...
    float3 finalLighting = 1;
        ...
        switch((3.0 /*_LightingMode*/))
        {
            ...
            case 3: // Skin
            {
                ...
                finalLighting = min(lerp(indirectLighting * float4(1,1,1,1), float4(1,1,1,1), _LightingIgnoreAmbientColor) + (sss * directLighting), directLighting);
            }
            break;
            case 4:
            {
