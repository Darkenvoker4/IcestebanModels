Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/f61555698aa953a4d9d20b8e6bdbd8ba"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_TEXTURERAMP 
			...
			void ApplyBackFaceColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, inout PoiMods poiMods)
			{
				if (!poiMesh.isFrontFace)
				{
					...
					backFaceColor.rgb = poiThemeColor(poiMods, backFaceColor.rgb, float(0));
					#if defined(PROP_BACKFACETEXTURE) || !defined(OPTIMIZER_ENABLED)
					backFaceColor *= POI2D_SAMPLER_PAN(_BackFaceTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0));
					#endif
					backFaceColor.rgb = hueShift(backFaceColor.rgb, frac(float(0) + float(0) * _Time.x) * float(0));
					float backFaceMask = 1;
					#if defined(PROP_BACKFACEMASK) || !defined(OPTIMIZER_ENABLED)
					backFaceMask *= POI2D_SAMPLER_PAN(_BackFaceMask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).r * float(1) * backFaceColor.a;
					#endif
					...
					poiFragData.emission += backFaceColor.rgb * float(0) * backFaceMask;
					poiMods.globalEmission = min(poiMods.globalEmission, float(1));
				}
			}
			#endif
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				mainShadowMask *= poiLight.shadowMask * float(0.9);
				firstSecondShadowMask *= poiLight.shadowMask * float(0.9);
				if (float(0) == 0)
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float shadowStrength = float(0.9) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,1,1,1) * lerp(indirectColor, 1, float(1))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.9));
				...
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - float(0.9));
				#endif
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(1));
				indirectColor = lerp(poiLight.directColor, indirectColor, float(0.9) * poiLight.shadowMask);
