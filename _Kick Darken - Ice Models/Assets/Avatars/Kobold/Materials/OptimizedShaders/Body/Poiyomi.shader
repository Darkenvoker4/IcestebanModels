Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/269da50bc98ad1e47bd20c2372256d94"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMASK 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,0,0.6520805,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,0,0.6520805,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,0,0.6520805,1).rgb, float(0));
				#endif
				if (float(0))
				{
				...
				#endif
				applyALEmmissionStrength(poiMods, emissionStrength0, float4(0,0,0,0), float(2), float4(0,10,0,0), float(2), float(1));
